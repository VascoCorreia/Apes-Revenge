...
local result = vector2.new(0,0)
if(vector2.magnitude(vec) ~= 0) then
result = vector2.div(vec, vector2.magnitude(vec))
end
return result
...
function vector2.limit(vec, max)
  ...
  if(mag > max) then
  return vector2.mult(vector2.normalize(vec), max)
  else
  return result
end
end
...
function vector2.addForce(force, mass, acceleration)
  ...
  return vector2.add(acc,acceleration)
end
